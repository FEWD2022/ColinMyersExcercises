...
function alphabetLength(array) {
  let arrayLength = array.length;
  return arrayLength;
}

// Print your answer to the dom
document.querySelector("#exercise-one").textContent =
  alphabetLength(theAlphabet);
...
thePlanets.push("XE-12-AER");

// Print your new array to the dom
document.querySelector("#exercise-two").textContent = thePlanets.join(", ");
...
for (let i = 0; i < snacks.length; i++) {
  if (snacks[i] === "nuts") {
    document.querySelector("#exercise-four").innerText =
      "My favorite snack is " + snacks[i];
  }
}
...
for (let i = 0; i < pets.length; i++) {
  ...
  newItem.innerText = pets[i];
  document.querySelector("#exercise-five").appendChild(newItem);
}
